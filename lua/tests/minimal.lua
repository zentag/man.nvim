...
if vim.uv.fs_stat(lazypath) then
  ...
  if not vim.wait(30000, function() return body ~= nil or request_err ~= nil end) then
    error('Timed out bootstrapping lazy.nvim')
  end
  if request_err then error(request_err) end
  local bootstrap, load_err = load(body)
  if not bootstrap then error(load_err) end
  bootstrap()
